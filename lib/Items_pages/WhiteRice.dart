...
class _WhiteRiceState extends State<WhiteRice> {
  ...
  Future<void> _loadData() async {
    ...
    setState(() {
      totalStock = prefs.getDouble('whiteRice_totalStock') ?? 0.0;
      totalReceived = prefs.getDouble('whiteRice_totalReceived') ?? 0.0;
      totalDispatched = prefs.getDouble('whiteRice_totalDispatched') ?? 0.0;

      String? transactionData = prefs.getString('whiteRice_transactions');
  ...
  Future<void> _saveData() async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      await prefs.setDouble('whiteRice_totalStock', totalStock);
      await prefs.setDouble('whiteRice_totalReceived', totalReceived);
      await prefs.setDouble('whiteRice_totalDispatched', totalDispatched);
      await prefs.setString('whiteRice_transactions', jsonEncode(transactions));
    } catch (e) {
      print("Error saving data: $e");
      _showError('حدث خطأ أثناء حفظ البيانات، يرجى المحاولة مرة أخرى.');
    }
  }
